...
export default function ClockWidget({ onAttendanceChange }) {
  ...
  const handleClockIn = async () => {
    ...
    try {
      const { data } = await api.post('/attendance/clock-in', { locationType });
      setRecord(data.record);
  ...
  if (loading) return <div className="clock-card"><div className="loading-spinner"><div className="spinner" /></div></div>;
  ...
  return (
    <div className="clock-card">
      <div className="clock-display">
        ...
        <span style={{ fontSize: '1.5rem', color: 'var(--text-secondary)', marginLeft: '8px' }}>
          {format(time, 'a')}
        </span>
      </div>
        ...
        <div className="status-item">
          ...
          <div>
            ...
            <div className="status-value">
              {record?.totalHours ? `${record.totalHours}h` : record?.inTime && !record?.outTime ? 'Active' : '—'}
            </div>
          </div>
        </div>
        {record?.locationType && (
          <div className="status-item">
            ...
            <div>
              ...
              <div className="status-value">{locationLabel[record.locationType]}</div>
            </div>
          </div>
        )}
      </div>
